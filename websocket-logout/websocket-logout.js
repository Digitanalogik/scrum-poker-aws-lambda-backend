// Scrum Poker API: WebSocket Logout - Tatu Soininen, 2023
// AWS Lambda function to remove a player from DynamoDB table
// and inform other players when somebody leaves the game.
...
export const handler = async (event) => {
  ...
  console.log("LAMBDA - WEBSOCKET LOGOUT - EVENT: ", event);
  ...
  if (player) {
    ...
    const message = `{"action":"player-logout","name":"${playerName}","id":"${id}"}`;
    ...
    try {
      ...
      return {
        statusCode: 500,
        body: JSON.stringify({ message: "Error while logging out player" }),
      };  
    }
